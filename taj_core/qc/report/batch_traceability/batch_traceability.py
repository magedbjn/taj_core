...
def execute(filters=None):
    ...
    trace_type = filters.get("trace_type") or "Manufacturing"
    ...
    siblings = []
    if include_bundle:
        siblings = _get_bundle_batches(batch) or []
        siblings = [x for x in siblings if x and x != batch]

    for root in produced_roots:
        wo_name = root.get("work_order") or ""
        se_name = root.get("stock_entry") or ""
        final_item = root.get("item_code") or ""
        final_qty = abs(float(root.get("qty") or 0))
        ...
        # ✅ Yellow sibling rows (Batch only) — once
        if siblings:
            for sib in siblings:
                data.append({
                    "level": 0,
...
def get_columns(tree_mode=False, include_sales=False):
    base = [
        ...
        # ✅ Supplier Source always visible
    ...
    if include_sales:
        base += [
            ...
            {
                "label": "Sales DocNo",
                "fieldname": "sales_docname",
                "fieldtype": "Dynamic Link",
                "options": "sales_doctype",
                "width": 180
            },
        ]
...
def build_sold_trace(final_batch: str):
    ...
    sle_rows += frappe.db.get_all(
        "Stock Ledger Entry",
        filters={**sle_filters, "batch_no": final_batch},
        fields=["voucher_type", "voucher_no", "actual_qty",
    ...
    if bundle_names:
        sle_rows += frappe.db.get_all(
            "Stock Ledger Entry",
            filters={**sle_filters, "serial_and_batch_bundle": ["in", bundle_names]},
            fields=["voucher_type", "voucher_no", "actual_qty",
                    "posting_date", "posting_time",
                    "serial_and_batch_bundle"],
            order_by="posting_date asc, posting_time asc"
    ...
    uniq = []
    for r in sle_rows:
        key = (r.get("voucher_type"), r.get("voucher_no"))
        if key in seen:
            continue
        seen.add(key)
        uniq.append(r)
    ...
    for sle in sle_rows:
        ...
        bname = sle.get("serial_and_batch_bundle")
        if bname:
            exact_qty = get_batch_qty_from_bundle([bname], final_batch, item_code=item_code)
            if exact_qty is not None:
                qty = abs(float(exact_qty))
...
@frappe.whitelist()
def get_bundle_batches(batch_no: str):
    return _get_bundle_batches(batch_no)


def _get_bundle_batches(batch_no: str):
...
def get_batch_qty_from_bundle(bundle_names, batch_no, item_code=None):
    total = 0.0
    found = False

    for bname in bundle_names:
        try:
            doc = frappe.get_doc("Serial and Batch Bundle", bname)
        except Exception:
            continue
        ...
        for r in rows:
            r_batch = getattr(r, "batch_no", None) or getattr(r, "batch", None)
            if r_batch != batch_no:
                continue
...
def get_production_roots_by_final_batch(final_batch: str):
    ...
    rows = frappe.db.get_all(
        ...
        fields=["parent as stock_entry", "item_code", "qty"],
    )
    ...
    if bundle_names:
        sed_rows = frappe.db.get_all(
            "Stock Entry Detail",
            filters={"serial_and_batch_bundle": ["in", bundle_names], "docstatus": 1, "t_warehouse": ["!=", ""]},
            fields=["parent as stock_entry", "item_code", "qty"]
        )
        for r in sed_rows:
            se_name = r.get("stock_entry")
            wo = frappe.db.get_value("Stock Entry", se_name, "work_order")
            exact_qty = get_batch_qty_from_bundle(bundle_names, final_batch, r.get("item_code"))
            roots.append({
                "item_code": r.get("item_code"),
    ...
    uniq = {}
    for x in roots:
        key = (x.get("stock_entry"), x.get("item_code"), final_batch)
        if key not in uniq:
            uniq[key] = x
        else:
...
def build_tree_view_for_batch(final_batch: str, produced_roots: list, max_depth: int, include_bundle: bool = False):
    ...
    siblings = []
    if include_bundle:
        siblings = _get_bundle_batches(final_batch) or []
        siblings = [x for x in siblings if x and x != final_batch]

    for i, root in enumerate(produced_roots, start=1):
        wo_name = root.get("work_order") or ""
        se_name = root.get("stock_entry") or ""
        qty = abs(float(root.get("qty") or 0))
        ...
        # ✅ Tree siblings under Final node (same as Table style)
        if siblings:

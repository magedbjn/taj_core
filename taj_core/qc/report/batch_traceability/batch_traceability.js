frappe.query_reports["Batch Traceability"] = {
  ...
  formatter: function (value, row, column, data, default_formatter) {
    ...
    // ✅ Sold: open Sales DocNo in new tab
    if (
      column.fieldname === "sales_docname" &&
      data.sales_doctype &&
      data.sales_docname
    ) {
    ...
    // ✅ Yellow sibling rows: show ONLY Batch column (works in Table & Tree)
    ...
    // Sold: keep default formatting (except above)
    if (trace_type === "Sold") {
      return value;
    }

    // Tree: keep default formatting (but allow sibling rows above)
    if (view_mode === "Tree (Expandable)") {
      return value;
    }

    // Section rows
    if (data.is_section_row === 1) {
      const bg = "background:#f0f0f0 !important;";
      const bold = "font-weight:800;";
      const pad = "padding:10px 12px;";
      const wrap = "white-space:normal; overflow:visible; text-overflow:unset; line-height:1.4;";
      if (column.fieldname === "item_name") {
        const title = frappe.utils.escape_html(data.item_name || "");
        return `<div style="${bg}${bold}${pad}${wrap} width:100%">${title}</div>`;
      }
      return `<div style="${bg}${pad}">&nbsp;</div>`;
    }
